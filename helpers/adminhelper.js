 ...
 /* -------------------------------------------------------------------------- */
  ...
  /*applyCoupon: (details, userId, date,totalAmount) => {
    return new Promise(async (resolve, reject) => {
          ...
          if (date <= expDate) {
            ...
            if (total >= coupon.minAmount) {
              ...
              if (total >= coupon.maxAmount) {
                ...
                response.maxAmountMsg = 'Maximum discount applicable to' + coupon.maxAmount
      ...
      if (response.dateValid && response.verifyminAmount &&response.maxAmount) {
      ...
      if (response.dateValid && response.verifymaxAmount && response.verifyminAmount) {
        response.maxamttrue=true
        response.maxamt=coupon.maxAmount
